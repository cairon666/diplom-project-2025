...
import (
	...
	"github.com/cairon666/vkr-backend/internal/api/www/rr_intervals_router"
	...
	"github.com/cairon666/vkr-backend/internal/repositories/rr_intervals_repo"
	...
	"github.com/cairon666/vkr-backend/internal/services/aggregation_service"
	...
	"github.com/cairon666/vkr-backend/internal/services/rr_intervals_service"
...
func main() {
	...
	fx.New(
		...
		fx.Provide(fx.Annotate(rr_intervals_repo.NewRRIntervalsRepo,
			fx.As(new(rr_intervals_service.RRIntervalsRepo)),
		)),
		fx.Provide(fx.Annotate(health_repo.NewHealthRepo,
			fx.As(new(health_service.HealthDataRepo)),
			fx.As(new(aggregation_service.HealthAggregationRepo)),
		)),
		// services
		...
		fx.Provide(fx.Annotate(device_service.NewDeviceService,
			...
			fx.As(new(rr_intervals_usecase.DeviceService)),
		)),
		fx.Provide(fx.Annotate(health_service.NewHealthService,
			fx.As(new(health_usecase.HealthService)),
		)),
		fx.Provide(fx.Annotate(aggregation_service.NewAggregationService,
			fx.As(new(health_usecase.AggregationService)),
		)),
		fx.Provide(fx.Annotate(rr_intervals_service.NewRRIntervalsService,
			fx.As(new(rr_intervals_usecase.RRIntervalsService)),
		)),
		// usecases
		...
		fx.Provide(fx.Annotate(rr_intervals_usecase.NewRRIntervalsUsecase, fx.As(new(rr_intervals_router.RRIntervalsUsecase)))),
		// www
		fx.Provide(
			...
			AsRoute(rr_intervals_router.NewRRIntervalsRouter),
			www.NewHTTPServer,
			fx.Annotate(
				www.NewServeMux,
...
func NewPgxPool(lc fx.Lifecycle, conf *config.Config) (postgres.PostgresClient, error) {
	pgClient, err := postgres.NewPostgres(context.Background(), conf.Database.PostgresURL)
	if err != nil {
		return nil, err
	}
...
func NewLogger(conf *config.Config) (logger.ILogger, error) {
...
func NewInfluxDBClient(lc fx.Lifecycle, conf *config.Config) (influxdb.InfluxDBClient, error) {
	// Увеличиваем timeout до 2 минут для тяжелых аналитических запросов
	influxClient := influxdb.NewInfluxDBWithTimeout(conf.InfluxDB.URL, conf.InfluxDB.Token, 2*time.Minute)
