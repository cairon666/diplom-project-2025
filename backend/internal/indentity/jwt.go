...
func (s *JWTService) CreateAccessToken(authClaims AuthClaims) (string, error) {
	...
	if err != nil {
		return "", apperrors.TokenCreationFailedf("failed to create access token: %v", err)
	}
...
func (s *JWTService) CreateRefreshToken(authClaims AuthClaims) (string, error) {
	...
	if err != nil {
		return "", apperrors.TokenCreationFailedf("failed to create refresh token: %v", err)
	}
...
func (s *JWTService) SetRefreshCookie(c *gin.Context, value string) {
	c.SetCookie("refresh_token", value, 60*60*24*30, "/", "*", false, true)
}
